...
function _processData(oImap,resp,error) {
		...
		try
		{
					...
					button.setContent(oImap.folders[0].list,numGoogle,oImap._conn._oAccount.get_account().presentation_identity);
...
function _initData() {
	...
	try {
		...
		for (let i=0; i < accounts.length; i++)
		{
			...
			if (  sprovider == "GOOGLE" || (sprovider=="WINDOWS LIVE" && config._safemode == 0) )
			{
